...
> 来源：https://www.yuque.com/xiumubai/doc/aohwxgclzzakhztk
...
​9123456789class MyComponent extends React.Component {  constructor(props) {    super(props);    this.myRef = React.createRef();  }  render() {    return <div ref="myref" />;  }}
访问当前节点的方式如下：

​91this.refs.myref.innerHTML = "hello";
传入对象

refs通过React.createRef()创建，然后将ref属性添加到React元素中，如下：

​9123456789class MyComponent extends React.Component {  constructor(props) {    super(props);    this.myRef = React.createRef();  }  render() {    return <div ref={this.myRef} />;  }}
当 ref 被传递给 render 中的元素时，对该节点的引用可以在 ref 的 current 属性中访问

​91const node = this.myRef.current;
传入函数
...
​JSXCopy912345678function App(props) {  const myref = useRef()  return (    <>      <div ref={myref}></div>    </>  )}
获取ref属性也是通过hook对象的current属性

​JavaScriptRun CodeCopy91const node = myref.current;
...
●[https://segmentfault.com/a/1190000020842342](https://segmentfault.com/a/1190000020842342)
