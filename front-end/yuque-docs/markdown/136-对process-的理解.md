...
> 来源：https://www.yuque.com/xiumubai/doc/enra0q03actf22ot
...
​91const args = process.argv.slice(2);
process.env
...
​JavaScriptRun CodeCopy9123456function foo() {    console.error('foo');}
process.nextTick(foo);console.error('bar');
输出结果为bar、foo

虽然下述方式也能实现同样效果：

​JavaScriptRun CodeCopy912setTimeout(foo, 0);console.log('bar');
两者区别在于：
