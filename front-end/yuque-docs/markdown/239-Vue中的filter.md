...
> 来源：https://www.yuque.com/xiumubai/doc/qkeplzg7n93ghz0g
...
​912345<!-- 在双花括号中 -->{{ message | capitalize }}
<!-- 在 `v-bind` 中 --><div v-bind:id="rawId | formatId"></div>
定义filter

在组件的选项中定义本地的过滤器

​91234567filters: {  capitalize: function (value) {    if (!value) return ''    value = value.toString()    return value.charAt(0).toUpperCase() + value.slice(1)  }}
定义全局过滤器：

​9123456789Vue.filter('capitalize', function (value) {  if (!value) return ''  value = value.toString()  return value.charAt(0).toUpperCase() + value.slice(1)})
new Vue({  // ...})
...
​912345import { indentity,resolveAsset } from 'core/util/index'
