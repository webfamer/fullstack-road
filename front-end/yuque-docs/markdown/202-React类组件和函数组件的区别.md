...
> 来源：https://www.yuque.com/xiumubai/doc/rqo2w0oklqwsczhl
...
​912345678class Welcome extends React.Component {  constructor(props) {    super(props)  }  render() {    return <h1>Hello, {this.props.name}</h1>  }}
二、函数组件

函数组件，顾名思义，就是通过函数编写的形式去实现一个React组件，是React中定义组件最简单的方式

​9123function Welcome(props) {  return <h1>Hello, {props.name}</h1>;}
...
​9123456789// 你的代码 class SayHi extends React.Component {     render() {         return <p>Hello, React</p >     } } // React内部 const instance = new SayHi(props) // » SayHi {} const result = instance.render() // » <p>Hello, React</p >
获取渲染的值
...
​JSXCopy9912345678910111213function ProfilePage(props) {  const showMessage = () => {    alert('Followed ' + props.user);  }
  const handleClick = () => {    setTimeout(showMessage, 3000);  }
  return (    <button onClick={handleClick}>Follow</button>  )}
类组件对应如下：

​JSXCopy9912345678910111213class ProfilePage extends React.Component {  showMessage() {    alert('Followed ' + this.props.user);  }
  handleClick() {    setTimeout(this.showMessage.bind(this), 3000);  }
  render() {    return <button onClick={this.handleClick.bind(this)}>Follow</button>  }}
...
●[https://juejin.cn/post/6844903806140973069](https://juejin.cn/post/6844903806140973069)
