...
15. ### What are middlewares in the context of NestJS?
    ...
    Here is a simple example of a logging middleware:
    ...
    - `console.log(...)` records basic information about the current request.
    - `req["requestTime"] = Date.now()` shows that middleware can attach extra fields to the request object, which can later be read by guards, interceptors, or controllers.
    - `next()` passes control to the next step in the request pipeline. If you do not call `next()` and also do not send a response manually, the request will hang.
...
19. ### What is the difference between interceptors and middleware?
    ...
    It becomes easier to understand if you compare `Middleware`, `Guard`, `Interceptor`, and `Pipe` around the same route. Suppose you have an endpoint like this:
    ...
    The request flow can be understood roughly like this:

    1. `Middleware`
       As soon as the request enters the application, middleware runs first. It is a good place for request logging, attaching common fields to the request object, handling CORS, and other HTTP-level concerns.
    2. `Guard`
       Before the route handler is executed, the guard decides whether the request is allowed to continue. For example, it may check whether the user is authenticated, whether a JWT is valid, or whether the user has the required role.
    3. `Interceptor (before)`
       The interceptor enters its `intercept()` method before the controller method runs. This is where you can log `Before...`, start a timer, or prepare some cross-cutting behavior.
    4. `Pipe`
       When Nest is about to pass arguments into the controller method, pipes transform and validate them. For example, `ValidationPipe` checks whether `dto` matches the DTO rules. If validation fails, an exception is thrown and the controller method will not run.
    5. `Controller / Service`
       Only after the previous checks pass does Nest actually execute `createUser()` and the service logic behind it.
    6. `Interceptor (after)`
       After the controller method returns, the logic attached after `next.handle().pipe(...)` runs. That makes interceptors ideal for wrapping responses, measuring execution time, and logging the outgoing result.

    In one sentence, their responsibilities are:

    - `Middleware`: HTTP-level preprocessing.
    - `Guard`: decides whether the request may continue.
    - `Pipe`: makes sure incoming data is valid and in the right shape.
    - `Interceptor`: wraps the whole execution and can work both before and after the handler.
